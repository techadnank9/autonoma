...
usage() {
  cat <<EOF
Usage:
  $SCRIPT_NAME --url URL --secret SECRET --scenario NAME [options]

Options:
  --test-run-id ID   Reuse a fixed test run id.
  --timeout SECONDS  Per-request timeout in seconds. Default: 30
  --skip-discover    Skip the discover call and go straight to up/down.
  --keep-up          Run up only. Do not call down.
  -h, --help         Show this help text.

Example:
  $SCRIPT_NAME \\
    --url https://example.com/api/autonoma \\
...
[ -n "$SCENARIO" ] || die "Missing --scenario"
